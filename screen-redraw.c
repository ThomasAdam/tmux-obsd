...
screen_redraw_draw_borders(struct client *c, int status, u_int top)
{
	...
	style_apply_update(&m_other_gc, oo, "pane-marked-border-style");

	memcpy(&m_active_gc, &active_gc, sizeof m_active_gc);
	style_apply_update(&m_active_gc, oo, "pane-marked-border-style");
	...
	for (j = 0; j < tty->sy - status; j++) {
		for (i = 0; i < tty->sx; i++) {
			...
			if (server_is_marked(s, s->curw, marked_window_pane) &&
				...
				if (active)
					tty_attributes(tty, &m_active_gc);
				else
					tty_attributes(tty, &m_other_gc);
			} else if (active)
